# RedcoralCommerce Database

## Overview

The `RedcoralCommerce` project contains the database schema and scripts to build the database.  
...
- **Scripts**  
  Contains a post deployment script `Script.PostDeployment.sql` and several database maintenance scripts  
...
```pwsh
sqlpackage /Action:Publish /SourceFile:"bin/Debug/RedcoralCommerce.dacpac" /TargetConnectionString:"Data Source=TONY23-PC\DEVSQL;Initial Catalog=RedcoralCommerce;Integrated Security=True;Persist Security Info=False;Pooling=False;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Command Timeout=30;"
```
...
|ErrorMessages.sql|Error messages for database|
